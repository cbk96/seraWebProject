...
스프링 프레임워크와 MariaDB로 구현하여 AWS로 배포한 포트폴리오용 뷰티 상품 온라인 쇼핑몰 프로젝트입니다.
<br><br>
...
PASSWORD : 1234
```
<p><img src="https://github.com/user-attachments/assets/f5a4de1b-193c-40b2-a37d-81b7ca7e78de" width="400px"></p>
고객용 페이지에서 테스트 계정으로 접속하면 페이지 우측 상단 메뉴에 관리자 페이지를 호출하는 버튼이 표시됩니다. 
<br><br>

## 📝 <u>프로젝트 주제 선정 이유</u>
주제를 선정하기 전 스스로의 개발 실력을 증진시킬 수 있는 프로젝트를 만들어 보고 싶다는 생각을 했었습니다.<br>
쇼핑몰은 상품이 등록되고 주문이 발생하는 과정에 이르기까지 다양한 CRUD 처리를 거치는 웹 서비스이며<br>
...
<details>
  ...
  <strong>적립</strong> : 마일리지 적립이 발생되게 되면 적립 금액은 DB의 tbl_mileage 테이블에 양수 값으로 저장 됩니다.<br>
  <strong>소진</strong> : 마일리지 소진이 발생되게 되면 사용된 액수가 DB의 tbl_mileage 테이블에 음수 값으로 저장됩니다. 현재 검색 내역을 통해 admin 회원이 2024년 7월 31일 8시 36분 57초에 4300원의 마일리지를 소진했음을 알 수 있습니다.
  <br><br>
  마이페이지등에서 회원이 보유중인 마일리지 총액을 계산하게 될 경우 DB의 tbl_mileage에서 해당 회원의 id로 검색되는 모든 적립액을 합산한 수치로 계산하기 때문에 소진액 만큼 차감된 결과를 얻을 수 있습니다.
  <br><br>
 </details>
...
<details>
 ...
 > <h4>상품 후기 작성</h4>
  ...
  마이페이지에서 배송완료 상태인 주문건의 후기작성 버튼을 클릭하면 상품 상세 페이지를 호출하여 상품 상세 페이지의 후기 작성란으로 이동하며 상품의 주문 이력이 있는 경우 후기를 작성하는게 가능하도록 설정 했습니다.
  <br><br>
