   	...
   	$(function(){
   		...
   		if(converMsg!=""){
   			if(converMsg=="converSuccess"){
   	   			alert("휴면회원해제를 완료되었습니다.");
   	   		}else{alert("휴면회원해제를 실패했습니다.");}
   		}
   		
   		
   		//////이벤트 실행문
   		btnModify.addEventListener("click", function(){
   			if(confirm("휴면회원 상태를 해제하시겠습니까?")){
   				modifyForm.submit();
   			}else{return;}
   		});
   	});
   ...
   <script src="/resources/js/searchOptionSettion.js"></script>
   	...
   	<div class="admin-section">
   		<ul class="admin-section-tab-bar">
   			<li class="now-tab">휴면회원 관리</li>
   		</ul>
   		
   		<div class="admin-section-content admin-section-content-default">
	   		<h3 class="content-title">휴면회원 관리</h3>
	   		<form action="/admin/member/sleeperOneModify"  method="post"  id="modify-form">
		   		<ul class="admin-info-view admin-info-view-default radio-visible">
		   			...
		   			<li class="confirm-btn-container">
		   				<button type="button" id="btn-modify" class="btn admin-btn-main" >휴면회원해제</button>
			   			<a href="/admin/member/memberList?active=no" class="btn btn-navy">목록</a>
			   		</li>
		   		</ul>
	   		</form>
